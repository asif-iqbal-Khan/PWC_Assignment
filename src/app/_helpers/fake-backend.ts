...
export class FakeBackendInterceptor implements HttpInterceptor {
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function generateRefreshToken() {
            ...
            // add token cookie that expires in 7 days (document.cookie not supported in StackBlitz so using localStorage)
        ...
        function getRefreshToken() {
            // get refresh token from cookie (document.cookie not supported in StackBlitz so using localStorage)
